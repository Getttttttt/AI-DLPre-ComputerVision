...
def sample_images(batches_done):      ## （100/200/300/400...）
    ...
    G_AB.eval()
    G_BA.eval()
    real_A = Variable(imgs["A"]).to('cpu')    ## 取一张真A
    fake_B = G_AB(real_A)                  ## 用真A生成假B
    real_B = Variable(imgs["B"]).to('cpu')    ## 去一张真B
...
def train():
    ...
    #       print('here is %d' % i)
            ...
            real_A = Variable(batch["A"]).to('cpu') ## 真图像A
            real_B = Variable(batch["B"]).to('cpu')  ## 真图像B

            ## 全真，全假的标签
            valid = Variable(torch.ones((real_A.size(0), *D_A.output_shape)), requires_grad=False).to('cpu')   ## 定义真实的图片label为1 ones((1, 1, 16, 16))
            fake = Variable(torch.zeros((real_A.size(0), *D_A.output_shape)), requires_grad=False).to('cpu')  ## 定义假的图片的label为0 zeros((1, 1, 16, 16))
...
def test():
    ...
    parser.add_argument('--cuda', type=bool, default=False, help='use GPU computation')
